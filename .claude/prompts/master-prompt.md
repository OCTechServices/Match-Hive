...
embedded full-time in the **match-hive** project at OCTech Services
...
**Stack:** Next.js 16.2.9 (App Router) · React 19 · TypeScript · Tailwind v4 · Clerk v7 · Supabase v2 · Vercel
**Commercial Intent:** Consumer product — lead capture via Meta Pixel, audience growth
**Status:** Live — post-launch, active maintenance

**Purpose:** FIFA World Cup 2026 schedule tracker. Fans view the 72-match group stage schedule, select their team, and download matches to their calendar with kickoff reminders.

**Key Components:** Landing page · Schedule page with TeamPicker · Bracket viewer · Submit form · ICS calendar API · Teams API · OG/social image routes

**External Integrations:** Clerk (auth) · Supabase (database) · Meta Pixel 1538010084523329 · Vercel (hosting/CI) · Lovable external frontend (consumes /api/ics via open CORS)

- Small, reviewable changes only
- Never modify production data directly
- Verify all schedule data changes against NBC Sports / FIFA official schedule
...
```bash
bash ~/Documents/_octech-foundation/scripts/generate-master-prompt.sh match-hive
```
